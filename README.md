...
HyperOptions is a local workstation for exploring covered calls and cash-secured puts on **Nasdaq-listed** stocks and watching selected option contracts. It is not a brokerage, does not store account or holdings data, and is not financial advice. The option chain and watchlist show market-implied odds when public quotes support a reliable estimate.
...
Before expiry, each standard call or put can show **market-implied ITM and OTM odds** on the chain and watchlist. These are risk-neutral estimates implied by option prices, not forecasts of the real-world frequency of an outcome. The event is the stock's regular-session close above the strike for a call, or below the strike for a put, on the expiry session. Equal close and strike is ATM. The backend fits one two-state `regimelib` distribution to validated call quotes for a ticker snapshot using the Treasury rate for each expiry, then prices a cash digital; complementary and put odds come from the same distribution. It publishes a number only when quote quality, model fit, numerical stability, dividend assumptions, and tight quote-derived probability bounds pass. Unsupported contracts, sparse quotes, unverified same-day quote timing, and incomplete evidence show **Odds unavailable**. The full reason appears in the watchlist card, the desktop chain row's **Why unavailable?** disclosure, or the expanded mobile row.

Visible chain pages refresh odds about every five minutes during the regular trading session; a visible watchlist polls for updated cached odds. After hours, a valid estimate from the latest session stays visible with its fetch and session dates until newer evidence is available. If Nasdaq has no usable bid and ask once that session is closed, the underlying for a new estimate is the official daily close of the latest completed session, not an older bar. Odds are held in a bounded, short-lived per-ticker cache; a restart may briefly show pending or unavailable while a new snapshot is assembled. The fetch time is a snapshot time, not a claim about each option's quote timestamp. The old strategy forecast snapshots remain in the local database for historical continuity but are never served as current market odds. **Check expiry results** requests a separate background close-based outcome update.
...
Watch jobs, watched contracts, close-based observations, and legacy forecast snapshots and model evidence are stored in separate tables of `.local/research/results.duckdb`. Legacy forecast bars and compiled indicators stay under `.local/research/`; current market odds use only the bounded in-memory cache. Set `STOCKSWEEPER_DATA_DIR` to use another local data directory. Existing manual Research run tables in an older local database are left untouched but are no longer used. These files are excluded from Git. Any leftover `.local/options.duckdb` or `.local/options.sqlite3` file is left untouched.
