...
HyperOptions is a local, MIT-licensed workstation for exploring covered calls and cash-secured puts on **Nasdaq-listed** stocks and watching selected option contracts. It is not a brokerage, does not store account or holdings data, and is not financial advice. StockSweeper's strategy engine runs automatically for watched contracts when enough validated market evidence is available.
...
The React browser client talks only to local FastAPI. Python is the only Nasdaq caller and caches option chains and stock quotes for 30 seconds and daily OHLCV for 24 hours. Computed contracts are reused while their inputs are unchanged. The Nasdaq-listed universe starts loading when the backend starts, refreshes daily, and fails closed when it cannot be loaded. Watchlist forecasts and expiry checks use Yahoo Finance data; see [third-party notices](THIRD_PARTY_NOTICES.md) for data-use and VectorBT dependency terms.
...
Watch jobs, watched contracts, close-based observations, forecast snapshots, and model evidence are stored in separate tables of `.local/research/results.duckdb`. Forecast bars and compiled indicators stay under `.local/research/`. Set `STOCKSWEEPER_DATA_DIR` to use another local data directory. Existing manual Research run tables in an older local database are left untouched but are no longer used. These files are excluded from Git. Any leftover `.local/options.duckdb` or `.local/options.sqlite3` file is left untouched.
